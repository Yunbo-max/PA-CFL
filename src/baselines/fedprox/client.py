# -*- coding: utf-8 -*-
# FedProx Client: FedAvg + proximal term mu/2 * ||w - w_global||^2
# Usage: python -m src.baselines.fedprox.client --client_number 5 --config configs/fedprox.json
...
parser.add_argument("--mu", type=float, default=0.01, help="Proximal term coefficient")
...
wandb.init(project=config["project_name"],
           config={"client_number": args.client_number, "region": region_name, "mu": args.mu})
...
class FedProxClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        global_params = [torch.from_numpy(p).float().clone() for p in parameters]
...
fl.client.start_numpy_client(
    server_address=config["server_address"], client=FedProxClient())
