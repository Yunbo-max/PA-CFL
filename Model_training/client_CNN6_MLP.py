...
# @Last Modified time: 2024-02-12 13:58:35
# -*- coding = utf-8 -*-
...
config = {"region": value}
wandb.init(project='Cambridge_sales_MLP_FL_west_8.4.2_8', config=config)
...
class RegressionClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config=None):
        ...
        wandb.log({'Scatter Plot': wandb.Image('scatter_plot6.png')})
        plt.close()
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig('distribution_plot6.png')
        wandb.log({'Distribution Plot': wandb.Image('distribution_plot6.png')})
        plt.close()
