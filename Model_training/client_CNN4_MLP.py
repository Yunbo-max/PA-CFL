...
config = {"region": value}
wandb.init(project='Cambridge_sales_MLP_FL', config=config)
...
class RegressionClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config=None):
        ...
        wandb.log({'Scatter Plot': wandb.Image('scatter_plot4.png')})
        plt.close()
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig('distribution_plot4.png')
        wandb.log({'Distribution Plot': wandb.Image('distribution_plot4.png')})
        plt.close()
