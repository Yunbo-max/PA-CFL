...
config = {"region": value}
wandb.init(project='Cambridge_sales_MLP_FL_west_8.4.2_4', config=config)
...
class RegressionClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config=None):
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig('scatter_plot17.png')
        wandb.log({'Scatter Plot': wandb.Image('scatter_plot17.png')})
        plt.close()
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig('distribution_plot17.png')
        wandb.log({'Distribution Plot': wandb.Image('distribution_plot17.png')})
        plt.close()
