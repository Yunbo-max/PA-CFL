...
config = {"region": value}
wandb.init(project='SCAIL3', config=config)
...
class RegressionClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config=None):
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig('scatter_plot11.png')
        wandb.log({'Scatter Plot': wandb.Image('scatter_plot11.png')})
        plt.close()
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig('distribution_plot11.png')
        wandb.log({'Distribution Plot': wandb.Image('distribution_plot11.png')})
        plt.close()
