...
config = {"region": value}
wandb.init(project='5_2_1', config=config)
...
class RegressionClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config=None):
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig('scatter_plot8.png')
        wandb.log({'Scatter Plot': wandb.Image('scatter_plot8.png')})
        plt.close()
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig('distribution_plot8.png')
        wandb.log({'Distribution Plot': wandb.Image('distribution_plot8.png')})
        plt.close()
